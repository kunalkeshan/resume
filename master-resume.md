...
**Auth & Security:** Auth0, Google Sign-In, Apple Sign-In, OAuth, Google reCAPTCHA v3

**AI/ML:** OpenAI API, IBM watsonx Orchestrate, adaptive filtering / signal processing (MATLAB), agentic assistant design
...
*(Ordered newest → oldest by primary build/activity date.)*

### 9wealth — Personal Finance / Wealth Management App (unlaunched)
**Freelance — Full-stack (mobile, backend, and infra)** | Sep 2025 – Present

- Built (not yet launched) a personal finance app for tracking budget, investments, assets, and net worth over time, with an AI chat interface for surfacing spending/investment trends in an easy, conversational flow.
- Built across the full stack: React Native/Expo mobile app (Supabase, TanStack Query, Google & Apple Sign-In, native UI primitives) and a NestJS/Prisma/Supabase API backend with OpenAI integration powering the AI trend-chat feature.
- Owned infrastructure for the project alongside frontend and backend feature work.
- Feature set includes nominee/asset management and allocation alerts, per the current build.
...
- Built a personal portfolio site for a maritime Navigation Officer, showcasing his career across container ships, Ro-Ro vessels, and international shipping routes.
- Live at [kumarsrivathsan.com](https://kumarsrivathsan.com).
...
- Built an agentic AI assistant that unifies scattered organizational knowledge (projects, tickets, policies, notes) into a single chat interface, powered by IBM watsonx Orchestrate.
- Designed a multi-agent architecture with an orchestrator that routes each question to the right specialist agent, with agents able to both read and take action (create tickets, add notes) via MCP tools — a closed loop from question to actioned outcome, not just Q&A.
- Built a supporting dashboard (Projects/Tickets/Policies tracking, plus direct per-agent chat access) alongside the core chat experience.
...
- Built the official website for hip-hop artist Sou, showcasing releases, music, and upcoming drops, with Sanity CMS for content management.
- Live at [keepingitsou.com](https://keepingitsou.com).
...
- Built (not yet launched) an e-commerce platform for an artisan dessert brand — handcrafted brownies, brookies, cupcakes, and cookies with delivery across Tamil Nadu.
- Full menu management, shopping cart, and product categorization with allergen/ingredient detail; Sanity CMS integration with on-demand revalidation via webhooks; transactional email templates via React Email.
- Built as a self-directed learning project — used as a testbed for Next.js 16/React 19, Zustand state management, and Sanity webhook-driven revalidation.
...
- Built a custom, server-hydrated video testimonial carousel for a single client, pulling content directly and natively from **2,500+ videos** uploaded into the client's Wix CMS — rather than requiring the client to manage a separate content source, they keep uploading from the one platform they already use.
- Embedded into the client's existing Wix site via an iframe/embed-code integration.
- Implemented lazy loading and infinite pagination for performance across the large video catalog.
